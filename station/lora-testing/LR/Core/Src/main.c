  ...
  while (1)
  	  {
	  	  ...
	  	  uint8_t buffer [70] = "Tw=21.9;Ta=22.6;Ma=37.7;Lo=2547;GN=50.088032;GE=14.452941;GT=102908\n\r";
	  	  //quickly send data

	  	  if(last_data_received){
	  		  uint8_t numdata_to_send = snprintf(data_tx,15, "LAST : %s\n\r", data_rx);
	  		  lora_send(data_tx, numdata_to_send, LORA_FHSS_ENABLED, LORA_IQ_INVERSION_ON);
